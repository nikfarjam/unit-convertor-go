...
func main() {
	...
	// Restricted to POST to prevent information disclosure or unintended execution via GET
	mux.HandleFunc("POST /converter", api.ConverterHandler)
	mux.HandleFunc("GET /version", api.VersionHandler)

	addr := ":9090"
	// Configured with explicit timeouts to mitigate Slowloris and other DoS attacks
	server := &http.Server{
		Addr:         addr,
		Handler:      mux,
